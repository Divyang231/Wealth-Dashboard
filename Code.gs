 ...
 *
 * It also reads the "Transactions" tab of a separate Wallet spreadsheet
 * (WALLET_SPREADSHEET_ID) to build the Passive Income card — entries
 * with Type = "Income", grouped by Txn_Category.
 *
...
// selector (3M/6M/1Y/All) slices and re-sums client-side so
// switching ranges doesn't need a reload. The sheet stays the
// single source of truth; this just filters rows by Type.
// ============================================================
function passiveIncome(walletTxns) {
  const out = [];
  walletTxns.forEach(t => {
    if (String(t.Type || '').trim().toLowerCase() !== 'income') return;
    const d = parseWalletDate(t.Date);
    if (!d) return;
    const amt = num(t.Income) || num(t.Amount);
    if (!amt) return;
    out.push({
      date: Utilities.formatDate(d, Session.getScriptTimeZone(), 'yyyy-MM-dd'),
      cat:  String(t['Txn_Category'] || '').trim() || 'Other',
      amt:  Math.round(amt)
    });
  });
  return out;
